...
function Header(ref: any, refBody: any, stateCheck: any) {
  ...
  useEffect(() => {
    const check = (Object.values(stateCheck).map((x: any) => x.refc) as boolean[]).includes(false) ? false : true
    const checkInderminate =
      Object.values(stateCheck).some((x: any) => x.refc) && Object.values(stateCheck).some((x: any) => !x.refc)
        ? true
        : false
