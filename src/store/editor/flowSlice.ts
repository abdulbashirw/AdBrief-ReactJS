  ...
  reducers: {
    ...
    addNode: (state, action: PayloadAction<any>) => {
      ...
      const newNode: Node = {
        ...
        type: "custom",
        position: {
          x: state.nodes.length
            ? state.nodes[state.nodes.length - 1].position.x +
              state.nodes[state.nodes.length - 1].measured.width +
              50
            : 100,
          y: state.nodes.length
            ? state.nodes[state.nodes.length - 1].position.y
            : 100,
        },
    ...
    removeElementsFromState(state, action: PayloadAction<DataNode[]>) {
      ...
      state.nodes = state.nodes.filter(
        (node: DataNode) => !deletedIds.includes(node.id),
      );
      state.edges = state.edges.filter(
        (edge: DataNode) => !deletedIds.includes(edge.id),
      );
    },
  },
});
