...
export default class Orchestrator {
  ...
  constructor(payor_code: string) {
    ...
    this.engine.setSystemMessage(
      SystemPrompts.KNOWLEDGE.replace("{payor_code}", payor_code),
    );
  }
  ...
  async process() {
    ...
    for (let i = 0; i < this.data.length; i++) {
      ...
      if (element.agen == "agent_chart") {
        element.chart_instruction.data =
          typeof tmpData == "string" ? JSON.parse(tmpData) : tmpData;
        agent.addData({ ...element });
      }
      console.log(element);
      console.log("========================================================");
